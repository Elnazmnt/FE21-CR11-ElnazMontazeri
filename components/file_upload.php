<?php
function file_upload($animal_photo) {
    $result = new stdClass();//this object will carry status from file upload
    $result->fileName = 'def.jpg';
    // if (isset($_SESSION['adm'])) {
    //     $result->fileName = 'product.png';
    // }
    $result->error = 1;//it could also be a boolean true/false
    //collect data from object $image
    $fileName = $animal_photo["name"];
    $fileType = $animal_photo["type"];
    $fileTmpName = $animal_photo["tmp_name"];
    $fileError = $animal_photo["error"];
    $fileSize = $animal_photo["size"];
